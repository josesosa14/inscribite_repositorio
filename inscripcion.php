...
*/
$result1=mysql_query('SELECT * FROM '.pftables.'usuarios WHERE dni="'.$_POST['dni'].'" LIMIT 1 ');
		...
		<div class="columnacentral" style="overflow:visible;">
			<div class="contenidoseccioncentral">
				Ya se encuentra registrado un usuario con ese DNI. Si olvidaste tu contraseña pulsa <a href="recordarpassword?recordar=password?username=<?=$_POST['dni']?>" style="text-decoration:underline;">aquí</a> y recibirás un correo electrónico para recordartelo.
				 Por cualquier consulta comunicate al 4641-4423 o por email con <a href="mailto:consultas@inscribiteonline.com.ar" style="text-decoration:underline;">consultas@inscribiteonline.com.ar</a>
			</div>
		</div>
<? } else {
...
while ($row1 = mysql_fetch_array($result1)) $idActual = $row1['id'];
...
while ($row1 =mysql_fetch_array($result1)) {
	...
	$result2=mysql_query('SELECT * FROM '.pftables.'categorias WHERE((deevento="'.$_GET['evento'].'")AND(opcion="'.$row1['opcion'.$iopciones].'")) ORDER BY codigo ');
	$result3=mysql_query('SELECT * FROM '.pftables.'usuarios WHERE dni="'.$usuario.'" LIMIT 1 ');
	$row3=mysql_fetch_array($result3);
	$sexodelusuario=$row3['sexo'];
    $fechanac=$row1['fechanac'];

	if (($row1['opcion'.$iopciones]!="")&&(mysql_num_rows($result2)>0)) { ?>
...
<?
    ...
    $fechdcomp=substr($fechadecomput,0,4)."/".substr($fechadecomput,4,2)."/".substr($fechadecomput,6,2);
		...
		if ((($row2['sexo']=='Ambos') || (strtolower($row2['sexo'])==strtolower($sexodelusuario)))&&((($edadcomputable>=$row2['edadminima'])&&($edadcomputable<=$row2['edadmaxima'])) || ($row2['limitedeedad']==0))&&(!$mascaro)) {
			if (gregoriantojd(date("m"),date("d"),date("Y"))==gregoriantojd(substr($row1['vencimiento3'],5,2),substr($row1['vencimiento3'],8,2),substr($row1['vencimiento3'],0,4))-1 ) { ?>
