<?	include_once 'includes/header.php';
	...
	$cat = $_GET['cat'];
	$cod = $_GET['cod'];
	$result1=mysql_query('SELECT * FROM '.pftables.'categorias WHERE ((nombre="'.$cat.'")AND(deevento="'.$evento.'")AND(codigo="'.$cod.'")) LIMIT 1 ');
...
<div class="left">
	...
	<h2>Ahora debes verificar que los datos que ingresaste sean correctos, por favor leelos atentamente y confirma la registracion.</h2>
	<form action="finregistro" method="post">
		...
		<div style="width:400px; height:325px; border:1px red solid; padding:10px; margin:10px auto;">
			...
			Teléfono Particular: <strong><?=$_POST['telefonoparticular']?></strong><br/>
			Teléfono Laboral: <strong><?=$_POST['telefonolaboral']?></strong><br/>
			Teléfono Celular: <strong><?=$_POST['telefonocelular']?></strong><br/>
