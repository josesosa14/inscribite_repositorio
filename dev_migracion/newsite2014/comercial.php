...
<?php } ?>
	...
	<select name="dianacimiento" id="dianacimiento" >
		<option></option>
		<?php for($i=1;$i<=31;$i++) { ?><option value="<?=$i?>" <?php if ((substr($row1['fechanac'],6,2)*1)==$i)echo'selected'?>><?=$i?></option> <?php } ?>
	</select>
	</p>
	<p>
	...
	<select name="agnonacimiento" id="agnonacimiento" onchange="cantidias();">
		<option></option>
		<?php for($i=date("Y");$i>=1900;$i--) { ?><option value="<?=$i?>"<?php if (substr($row1['fechanac'],0,4)==$i)echo' selected="selected"'?>><?=$i?></option> <?php } ?>
