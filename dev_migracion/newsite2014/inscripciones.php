...
$cuentaev=0;
$result1=mysql_query('SELECT * FROM '.pftables.'eventos WHERE ver=1 AND tipo="'.$_GET['tipo'].'" ORDER BY fechaord, nombre LIMIT '.$limitdesde.', '.$paginardea);
while ($row1 =mysql_fetch_array($result1)) {
/*
$cuentaev++;
       ?><div class="cadaevento">
          <div class="titulo">
           <div><a href="iniciainscri?evento=<?=$row1['codigo']?>"><?=$row1['nombre']?></a></div>
          </div>
...
$datos=getimagesize("imagenes/image_".$row1['imagen1']); ?>
           <div style="width:160px;display:inline;float:left;margin-left:107px;text-align:center;overflow:hidden;">
            <a href="iniciainscri?evento=<?=$row1['codigo']?>">
             <img src="imagenes/image_<?=$row1['imagen1']?>" alt="<?=$row1['nombre']?>" style="width:<?=$datos[0]?>px;height:<?=$datos[1]?>px;display:inline;"/>
            </a>
           </div>
<?php }
if (file_exists("imagenes/image_".$row1['imagen2'])) { ?>
           <a href="iniciainscri?evento=<?=$row1['codigo']?>">
            <img src="imagenes/image_<?=$row1['imagen2']?>" alt="<?=$row1['nombre']?>" style="display:inline;float:left;clear:none;margin-right:13px;margin-left:9px;"/>
           </a>
<?php } ?>
          </div>

          <div class="descripciondeevento">
           <?=preg_replace("(\r\n|\n|\r)","<br/>",$row1['descripcion']).chr(13);?>
          </div>

          <div class="botoninscripcion">
           <div>
            <div style="margin-bottom:6px;margin-bottom:0px;padding-bottom:0px;"><a href="competidoresinscriptos?evento=<?=$row1['codigo']?>">Ver Competidores Inscriptos</a></div>
           </div>
          </div>
         </div><?
*/
...
$cuentaev++;
       ...
       <tr class="<?=($cuentaev%2==0)?'lineaspares':'lineasimpares'?>">
        <td>
         <a href="evento?evento=<?=$row1['codigo']?>"><?=$row1['codigo']?></a>
        </td>
        <td>
         <a href="evento?evento=<?=$row1['codigo']?>"><?=($row1['fecha']!='')?$row1['fecha']:'&nbsp;'?></a>
        </td>
        <td>
         <a href="evento?evento=<?=$row1['codigo']?>"><?=$row1['nombre']?></a>
        </td>
        <td>
         <a href="evento?evento=<?=$row1['codigo']?>"><?=$row1['empresa']?></a>
        </td>
        <td style="width:65px;">
         <a href="evento?evento=<?=$row1['codigo']?>" style="color:#EC126C;text-decoration:underline;width:65px;">Más Info</a>
        </td>
       </tr>
<?php } ?>
         </table>
...
<?php for($cpag=0;$cpag<$cantitems/$paginardea;$cpag++) { ?>
...
<?php } */ ?>
         <span style="float:right;clear:none;margin-top:10px;"><?php /*<a href="eventoslistacompleta" style="text-decoration:underline;">Ver como lista</a>*/ ?></span>
        </div>
       </div>
      </div>
