...
<?php $result2 = mysql_query('SELECT opcion FROM '.pftables.'categorias WHERE ((deevento = "'.$row1['deevento'].'") AND (nombre = "'.$row1['categoria'].'")) LIMIT 1');
...
<?php } ?>
			...
			<div>
				...
				<?php if($row1['pagado'] != 1):?>	<a href="imprimircupon?evento=<?=$row1['deevento']?>&amp;cat=<?=$row1['categoria']?>&amp;cod=<?=substr($row1['codigo'],4,2)?>&amp;reimpr=1">Reimprimir cupón de pago</a><br/>
