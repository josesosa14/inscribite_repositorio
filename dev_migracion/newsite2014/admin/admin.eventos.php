...
<?php /* 					<th style="text-align:left;width:38px;">Nro</th> */ ?>
            ...
            if (isset($_GET['evento'])) {
                ...
                $result1 = mysql_query('SELECT * FROM inscribite_eventos WHERE id > 0 AND codigo = ' . $evento_id);
            } else {
                $result1 = mysql_query('SELECT * FROM inscribite_eventos WHERE id > 0' . $filtroportipo . $filtrover . ' ORDER BY orden ');
            }
            while ($row1 = mysql_fetch_array($result1)) {
                $contnrodeid++;
                $result2 = mysql_query('SELECT * FROM empresa WHERE emp_nombre= LOWER("' . $row1['empresa'] . '") LIMIT 1 ');
                    ...
                    <td>
                        <div id="contorden<?= $contnrodeid ?>_8">
                            <a href="?sec=categorias.admin&amp;evento=<?= $row1['codigo'] ?>" style="font-family:monospace;cursor:pointer;">
                                <span style="float:left;cursor:pointer;"> <?= mysql_num_rows(mysql_query('SELECT * FROM inscribite_categorias WHERE deevento = "' . $row1['codigo'] . '"')) ?>
    ...
    <?php /* =agcnbsp(mysql_num_rows(mysql_query('SELECT * FROM inscribite_inscripciones WHERE deevento = "'.$row1['codigo'].'"')),4) */ ?>
    <?= mysql_num_rows(mysql_query('SELECT * FROM inscribite_inscripciones WHERE deevento = "' . $row1['codigo'] . '"')) ?>
                            </a>
                        </div>
                    </td>
    ...
    <?php /* 							(<?=mysql_num_rows(mysql_query('SELECT * FROM inscribite_inscripciones WHERE deevento = "'.$row1['codigo'].'" AND pagado = 1 AND iniciadoeldia != "0000-00-00" '))?>) */ ?>
                            </a>
                        </div>
                    </td>
                    <td>
