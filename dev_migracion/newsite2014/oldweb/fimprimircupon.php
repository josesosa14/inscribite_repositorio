...
$diadenacusuario=$row['dianacimiento'];
$mesdenacusuario=$row['mesnacimiento'];
$agnodenacusuario=$row['agnonacimiento'];
$dniinscripto=$row['dni'];
$dniinscripto=agceros($dniinscripto,8);
$maildelusuario=$row['email'];
if(strlen($dniinscripto)==0)echo'<script type="text/javascript">location.href="http://www.inscribiteonline.com.ar/iniciainscri?evento='.$_GET['evento'].'"; </script>';
...
if(mysql_num_rows($result1)==0)
    $result1=mysql_query('SELECT * FROM inscribite_categorias WHERE((nombre="'.$_GET['cat'].'")AND(deevento="'.$_GET['evento'].'")) LIMIT 1 ');
while($row=mysql_fetch_array($result1)){
	...
	if((gregoriantojd($mesactual,$diaactual,$agnoactual)<gregoriantojd(substr($row['vencimiento1'],5,2),substr($row['vencimiento1'], 8, 2),substr($row['vencimiento1'],0,4)))||($row['vencimiento1']=='0000-00-00')){
    //if($fechaactual*1<$row['fechavenc1']*1){
    	$precioarray=split(',',$row['precio1']);
		$precio2dovarray=split(',',$row['precio2']);
		$diasparav=gregoriantojd(substr($row['vencimiento1'],5,2),substr($row['vencimiento1'],8,2),substr($row['vencimiento1'],0,4))-gregoriantojd(1,1,$agnoactual);
		$diasparav++;
		$desdeel1ervh2do=gregoriantojd(substr($row['vencimiento2'],5,2),substr($row['vencimiento2'],8,2),substr($row['vencimiento2'],0,4))-gregoriantojd(substr($row['vencimiento1'],5,2),substr($row['vencimiento1'],8,2),substr($row['vencimiento1'],0,4));
        //$desdeel1ervh2do++;
        //echo $desdeel1ervh2do;
		$desdeel1ervh2do=agceros($desdeel1ervh2do,2);

		$diaquevence=substr($row['vencimiento1'],8,2);
		$mesquevence=substr($row['vencimiento1'],5,2);
		$agnoquevence=substr($row['vencimiento1'],0,4);
		if(($diaquevence-1)==0){
        ...
        if($row['vencimiento1']=='0000-00-00'){
		    $mesquevence=date('m');
    ...
    }elseif(gregoriantojd($mesactual,$diaactual,$agnoactual)<gregoriantojd(substr($row['vencimiento2'],5,2),substr($row['vencimiento2'],8,2),substr($row['vencimiento2'],0,4))){
    //}elseif($fechaactual*1<$row['fechavenc2']*1){

		//"Antes del 2do vencimiento";
		$precioarray=split(',',$row['precio2']);
		$precio2dovarray=split(',',$row['precio3']);
		$diasparav=gregoriantojd(substr($row['vencimiento2'],5,2),substr($row['vencimiento2'],8,2),substr($row['vencimiento2'], 0, 4))-gregoriantojd(1,1,$agnoactual);
		$diasparav++;
		$desdeel1ervh2do=gregoriantojd(substr($row['vencimiento3'],5,2),substr($row['vencimiento3'],8,2),substr($row['vencimiento3'],0,4))-gregoriantojd(substr($row['vencimiento2'],5,2),substr($row['vencimiento2'],8,2),substr($row['vencimiento2'],0,4));
		//$desdeel1ervh2do++;
		$desdeel1ervh2do=agceros($desdeel1ervh2do,2);
		$diaquevence=substr($row['vencimiento2'],8,2);
		$mesquevence=substr($row['vencimiento2'],5,2);
		$agnoquevence=substr($row['vencimiento2'],0,4);
		if(($diaquevence-1)==0){
			$diaquevence=$diaquevence=(gregoriantojd($mesquevence,1,$agnoactual)-gregoriantojd($mesquevence-1,1,$agnoactual))+1;
			$mesquevence--;
		}
		...
		$mesquevence=agceros($mesquevence,2);
		$fechaquevence=$diaquevence."/".$mesquevence."/".$agnoquevence;
	}elseif(gregoriantojd($mesactual,$diaactual,$agnoactual)<gregoriantojd(substr($row['vencimiento3'],5,2),substr($row['vencimiento3'],8,2),substr($row['vencimiento3'],0,4))){
    //}elseif($fechaactual*1<$row['fechavenc2']*1){
    	//Antes del 3er vencimiento
		$precioarray=split(',',$row['precio3']);
		$precio2dovarray=split(',',$row['precio3']);
		$diasparav=gregoriantojd(substr($row['vencimiento3'],5,2),substr($row['vencimiento3'],8,2),substr($row['vencimiento3'],0,4))-gregoriantojd(1,1,$agnoactual);
		$diasparav++;
		$desdeel1ervh2do=00;
		$diaquevence=substr($row['vencimiento3'],8,2);
		$mesquevence=substr($row['vencimiento3'],5,2);
		$agnoquevence=substr($row['vencimiento3'],0,4);
		if(($diaquevence-1)==0){
			$diaquevence=$diaquevence=(gregoriantojd($mesquevence,1,$agnoactual)-gregoriantojd($mesquevence-1,1,$agnoactual))+1;
			$mesquevence--;
		}
...
$result3=mysql_query('SELECT * FROM inscribite_descuentos WHERE codevento='.$_GET['evento'].' AND coddni='.$usuario.' LIMIT 1 ');
...
$yaseinscribio=false;
$result1=mysql_query('SELECT id FROM inscribite_inscripciones WHERE((deusuario="'.$dniinscripto.'")AND(deevento="'.$_GET['evento'].'")AND(categoria="'.$_GET['cat'].'")AND(codigo="'.$idevento.$categoria.$edadcomputable.$dniinscripto.'")) LIMIT 1 ');
...
if(!($yaseinscribio)){
	...
	mysql_query("UPDATE inscribite_inscripciones SET venceeldia=\"".date("Ymd",(mktime(1,1,1,date("m"),date("d"),date("Y"))+60*60*24*7))."\" WHERE id=$idActual");
...
<html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <div id=factura align="center">
                    <table width="630" align="center" cellspacing="0">
                      ...
                      <tr>
                        ...
                        <td width="164">
                          <div align="center">
                            <img src="webimages/inscribite_fac.jpg" alt="d" width="164" height="70" align="middle"/>
                            <br/>
                            <br/>
                            <br/>
                            <br/>
  ...
  <?php for($i=0;$i<count($a);$i++)echo$spa[$i].$a[$i]?>
                          </span>
                        </td>
                      </tr>
                    </table>
            ...
            <td colspan="2" class="Estilo1"><div align="center">
              <table cellspacing="0" width="150" style='width:112.5pt'>
                <tr>
                  ...
                  <td>
                    <p>
                      <a href="http://www.inscribiteonline.com.ar/">
                        <strong>
