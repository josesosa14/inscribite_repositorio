...
<div class="col-xs-12 col-sm-6 col-md-5 pull-right login-container">
    <?php if (empty($usuario)) : ?>
        ...
        <form method="POST" action="altas/login.php" id="formx">
                ...
                $(document).ready(function () {
                    $("#recuperarContraseña").click(function (evento) {
                        ...
                        if (dniUsuario.length >= 7 && dniUsuario.length <= 8 && $.isNumeric(dniUsuario)) {
                            ...
                            respuesta = $.ajax({
                                type: "POST",
                                url: '<?= base_url() ?>general/reenviar_password.php',
            ...
            <!-- BOX DE RECUPERAR CONTRASEÑA -->
            <div class="left">
                <div><a href="<?=base_url()?>login.php" id="recuperarContraseña" style="color:grey">Olvidaste tu contraseña?</a></div>
                <a href="<?=base_url()?>unirse.php">No tengo cuenta. Registrarme</a>
            </div>
            <div class="right">
        ...
        <script>
            ...
            $("#btActions").click(function (evento) {
                ...
                if (dniUsuario.length >= 7 && dniUsuario.length <= 8 && $.isNumeric(dniUsuario)) {
                    ...
                    respuesta = $.ajax({
                        type: "POST",
                        url: '<?= base_url() ?>general/buscar_dni.php',
                        ...
                        async: false
                            ...
                            .done(function (msg) {
                                if (msg == 1) {
                                    ...
                                    $("#mensaje3").empty();
                                    $("#mensaje3").append('<span style="color:black">Para registrarse haga click <a class="blue" href="http://www.inscribiteonline.com.ar/unirse.php?dni=' + dniUsuario + '">aquí<a/></span>');
                                    $('.modalMessage').modal();
                                }
                            });
                } else {
    ...
    <?php else : ?>
        ...
        <div class="row">
            <a href="<?=base_url()?>miCuenta.php">Mi cuenta</a>
            &nbsp;&nbsp;&nbsp;
            <a href="<?=base_url()?>pagar.php#toShow">Inscripci&oacute;n a eventos</a>
        </div>
    <?php endif; ?>
</div>
