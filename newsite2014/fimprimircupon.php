...
if (strlen($dniinscripto) == 0)
    echo '<script type="text/javascript">location.href="' . url . 'iniciainscri?evento=' . $_GET['evento'] . '"; </script>';
...
while ($row1 = mysql_fetch_array($result1)) {
    ...
    $spa[$z] = " ";
    // 6: monto enteros
    $a[$z] = substr($montopesos, 0, 1);
    $z++;
    $a[$z] = substr($montopesos, 1, 1);
    $z++;
    $a[$z] = substr($montopesos, 2, 1);
    $z++;
    $a[$z] = substr($montopesos, 3, 1);
    $z++;
    $a[$z] = substr($montopesos, 4, 1);
    $z++;
    $a[$z] = substr($montopesos, 5, 1);
    $z++;
    $spa[$z] = " ";
    // 2: monto decimales
    $a[$z] = substr($montocenta, 0, 1);
    $z++;
    $a[$z] = substr($montocenta, 1, 1);
    $z++;
    $spa[$z] = " ";
    // fecha primer vencimiento AAJJJ (JJJ son la cantidad de dias desde el 1 de enero del ao corriente
    $a[$z] = substr($agnoactual, 2, 1);
    $z++;
    $a[$z] = substr($agnoactual, 3, 1);
    $z++;
    $a[$z] = substr($diasparav, 0, 1);
    $z++;
    $a[$z] = substr($diasparav, 1, 1);
    $z++;
    $a[$z] = substr($diasparav, 2, 1);
    $z++;
    $spa[$z] = " ";
    // id del cliente (DNI?) 15 cifras o 14 ???
    $a[$z] = substr($idcliente, 0, 1);
    $z++;
    $a[$z] = substr($idcliente, 1, 1);
    $z++;
    $a[$z] = substr($idcliente, 2, 1);
    $z++;
    $a[$z] = substr($idcliente, 3, 1);
    $z++;
    $a[$z] = substr($idcliente, 4, 1);
    $z++;
    $a[$z] = substr($idcliente, 5, 1);
    $z++;
    $a[$z] = substr($idcliente, 6, 1);
    $z++;
    $a[$z] = substr($idcliente, 7, 1);
    $z++;
    $a[$z] = substr($idcliente, 8, 1);
    $z++;
    $a[$z] = substr($idcliente, 9, 1);
    $z++;
    $a[$z] = substr($idcliente, 10, 1);
    $z++;
    $a[$z] = substr($idcliente, 11, 1);
    $z++;
    $a[$z] = substr($idcliente, 12, 1);
    $z++;
    $a[$z] = substr($idcliente, 13, 1);
    $z++;
    $spa[$z] = " ";
    ...
    // monto enteros
    $a[$z] = substr($montoven1, 0, 1);
    $z++;
    $a[$z] = substr($montoven1, 1, 1);
    $z++;
    $a[$z] = substr($montoven1, 2, 1);
    $z++;
    $a[$z] = substr($montoven1, 3, 1);
    $z++;
    //$a[$z] = "0";$z++;
    //$a[$z] = "0";$z++;
    $spa[$z] = " ";
    ...
    $a[$z] = substr($montoven1cent, 0, 1);
    $z++;
    $a[$z] = substr($montoven1cent, 1, 1);
    $z++;
    $spa[$z] = " ";
    ...
    //$a[$z] = "0"; $z++;
    //$a[$z] = "0"; $z++;
    $a[$z] = substr($desdeel1ervh2do, 0, 1);
    $z++;
    $a[$z] = substr($desdeel1ervh2do, 1, 1);
    $z++;
    $spa[$z] = " ";
    $secuencia = array();
    $secuencia[0] = 9;
                            ...
                            for ($i = 0; $i < count($b); $i+=2) {
                                if ($i <= count($b)) {
                                    for ($n = 0; $n < $barcode1[$b[$i]]; $n++)
                                        echo'           <img src="webimages/negro.gif"  alt="" hspace="0"/>' . chr(13);
                                    for ($n = 0; $n < $barcode1[$b[$i + 1]]; $n++)
                                        echo'           <img src="webimages/blanco.gif" alt="" hspace="0"/>' . chr(13);
