...
if ($_GET['eventos'] != '') {
	...
	$result1 = mysqli_query($coneccion,"SELECT * FROM inscribite_eventos WHERE $busqueda ");
}
while ($row1 = mysqli_fetch_array($result1)) {
$tipo = $row2['tipo'];
$result3 = mysqli_query($coneccion,'SELECT id FROM inscribite_inscripciones WHERE deevento = "'.$row1['codigo'].'" ');
$cantproductos = mysqli_num_rows($result3);
if ($_GET['busqueda'] == '') {
  $ordenarpor = $_GET['ordenarpor'];
  if ($ordenarpor == '') {
	if ($ordenarinscripcionesx == '') {
		$result2 = mysqli_query($coneccion,'SELECT * FROM inscribite_inscripciones WHERE deevento = "'.$row1['codigo'].'" LIMIT '.$limitdesde.', '.$paginardea.' ');
	} else {
		if ($ordenarinscripcionesx == 'fecha') $ordenarinscripcionesx = 'pagoeldia';
		$result1 = mysqli_query($coneccion,'SELECT * FROM inscribite_inscripciones WHERE deevento = "'.$row1['codigo'].'" ORDER BY '.$ordenarinscripcionesx.' LIMIT '.$limitdesde.', '.$paginardea.' ');
	}
  } else {
	$result2 = mysqli_query($coneccion,'SELECT DISTINCT * FROM inscribite_inscripciones LEFT OUTER JOIN inscribite_usuarios ON inscribite_usuarios.dni = inscribite_inscripciones.deusuario WHERE deevento = "'.$row1['codigo'].'" ORDER BY inscribite_usuarios.'.$ordenarpor.' LIMIT '.$limitdesde.', '.$paginardea.' ');
  }
} else {
  $result2 = mysqli_query($coneccion,'SELECT * FROM inscribite_inscripciones WHERE (( deusuario LIKE "%'.$_GET['busqueda'].'%") OR ( deusuario = '.agceros($_GET['busqueda'],8).' )) ');
  //echo 'SELECT * FROM inscribite_inscripciones WHERE (( deusuario LIKE "%'.$_GET['busqueda'].'%") OR ( deusuario = '.agceros($_GET['busqueda'],8).' )) ';
}
while ($row2 = mysqli_fetch_array($result2)) {
  $result3 = mysqli_query($coneccion,'SELECT * FROM inscribite_usuarios WHERE dni = "'.agceros($row2['deusuario'],8).'" OR dni = "'.($row2['deusuario']*1).'" LIMIT 1 ');
  while ($row3 = mysqli_fetch_array($result3)) {
	...
	$result4 = mysqli_query($coneccion,'SELECT * FROM inscribite_categorias WHERE (( deevento = "'.$row2['deevento'].'" ) && ( codigo = "'.substr($row2['codigo'], 4, 2).'" )) LIMIT 1 ');
	...
	if (is_numeric($row2['categoria'])) {
	  $result5 = mysqli_query($coneccion,'SELECT * FROM inscribite_categorias WHERE deevento = "'.$row2['deevento'].'" AND codigo = "'.$row2['categoria'].'" LIMIT 1 ');
...
if (is_resource($result4)) mysqli_free_result($result4);
if (is_resource($result5)) mysqli_free_result($result5);
