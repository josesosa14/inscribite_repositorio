...
mysqli_query($coneccion,'UPDATE inscribite_inscripciones SET selemandomail = 1 WHERE codigo = "'.$_GET['cod'].'" ');

$result1 = mysqli_query($coneccion,'SELECT * FROM inscribite_inscripciones WHERE codigo = "'.$_GET['cod'].'" LIMIT 1 ');
while ($row1 = mysqli_fetch_array($result1)) {

//print_r($row1);

	$result2 = mysqli_query($coneccion,'SELECT * FROM inscribite_usuarios WHERE dni = "'.substr($_GET['cod'],-8).'" LIMIT 1 ');
	while ($row2 = mysqli_fetch_array($result2)) {

		$result3 = mysqli_query($coneccion,'SELECT nombre FROM inscribite_eventos WHERE codigo = "'.$row1['deevento'].'" LIMIT 1 ');
