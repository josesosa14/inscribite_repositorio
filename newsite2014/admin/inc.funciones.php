...
foreach ($_GET as $nombrevariable => $valorvariable) {
    ...
    if ($nombrevariable != 'pagina')
        $uriactualsinpag .= $nombrevariable . '=' . $valorvariable . '&amp;';
}
...
function paginacion($nombretabla = '', $strquery = '',$coneccion) {
    ...
    if (($ultimoQueryYtabla != $nombretabla . $strquery)) {
        ...
        $result1 = mysqli_query($coneccion,'SELECT ' . pftables . $nombretabla . '.id FROM ' . pftables . $nombretabla . (($strquery == '') ? '' : ' WHERE ' . $strquery));
        $cantitems = mysqli_num_rows($result1);
        $ultenpag = ($_GET['pagina'] * $cantidadporpag);
        if ($ultenpag > $cantitems)
            $ultenpag = $cantitems;

        $txpaginacion = '<div class="paginacion">';
        if ($_GET['pagina'] > 2)
            $txpaginacion .= '<a href="' . str_replace('?', '', $uriactualsinpag) . '">« Primera página</a>';
        if ($_GET['pagina'] > 1)
            $txpaginacion .= ' <a href="' . $uriactualsinpag . 'pagina=' . ($_GET['pagina'] - 1) . '" style="font-weight:bold;">‹ Anterior</a> ';
        $txpaginacion .= (($_GET['pagina'] - 1) * $cantidadporpag + 1) . ' - ' . $ultenpag . ' de ' . $cantitems;
        if ($ultenpag != $cantitems)
            $txpaginacion .= ' <a href="' . $uriactualsinpag . 'pagina=' . ($_GET['pagina'] + 1) . '" style="font-weight:bold;">Siguiente ›</a>';
...
function LogIn($coneccion) {
    ...
    if ((isset($_POST['alogin_username'])) && (isset($_POST['alogin_password']))) {
        ...
        $result1 = mysqli_query($coneccion,$q = 'SELECT * FROM ' . pftables . 'admin_users WHERE (username = "' . $alogin_username . '" OR email = "' . $alogin_username . '") AND password = "' . chash($_POST['alogin_password']) . '" LIMIT 1');
        if ($logged = mysqli_fetch_array($result1)) {
            $loginhash = chash($alogin_username . $http_client_ip . time());
            setcookie("alogin_hash", $loginhash, 0, "/");

            mysqli_query($coneccion,"UPDATE " . pftables . "admin_users SET sesionanterior = '" . time() . "' WHERE id = " . $logged['id'] . " LIMIT 1;");
            mysqli_query($coneccion,"INSERT INTO " . pftables . "sesiones ( `id`, `user`, `hash`, `address`, `lastlogin` ) VALUES ( NULL , '" . $logged['id'] . "', '" . chash($localPass . $loginhash) . "', '" . $http_client_ip . $_SERVER['REMOTE_ADDR'] . "', '" . time() . "' );");
            session_start();
            $_SESSION['admin'] = 'inscribite';
            cerrarDb($coneccion);
    ...
    if ($logged['id'] == '') {
        ...
        if ($loginhash != '') {
            ...
            if (strpos($_SERVER['HTTP_USER_AGENT'], 'Validator') > 0) {
                $result1 = mysqli_query($coneccion,'SELECT ' . pftables . 'sesiones.*, ' . pftables . 'admin_users.username, ' . pftables . 'admin_users.email, ' . pftables . 'admin_users.sesionanterior FROM ' . pftables . 'sesiones JOIN ' . pftables . 'admin_users ON ' . pftables . 'sesiones.user = ' . pftables . 'admin_users.id LIMIT 1');
                $logged = mysqli_fetch_array($result1);
                return true;
            }
            if ($loginSiNo) {
                cerrarDb($coneccion);
