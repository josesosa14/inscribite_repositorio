...
function LogIn(){
	...
	if ((isset($_POST['alogin_username'])) && (isset($_POST['alogin_password']))) {
		...
		$result1 = mysqli_query($coneccion,'SELECT * FROM '.pftables.'admin_users WHERE (username = "'.$alogin_username.'" OR email = "'.$alogin_username.'") AND password = "'.chash($_POST['alogin_password']).'" LIMIT 1');
		if ($logged = mysqli_fetch_array($result1)) {
			...
			$msg = '';
			/*$result1 = mysqli_query($coneccion,'SELECT * FROM '.pftables.'registro WHERE fecha != '.date('Ymd'));
			...
			/*if (mysqli_num_rows($result1)) {
				...
				$mail->Send();
				mysqli_query($coneccion,"TRUNCATE TABLE `".pftables."registro` ");
			} */
			//
			cerrarDb();
...
function cerrarDb(){
	...
	if (isset($result1)){ if (is_resource($result1)) mysqli_free_result($result1); }
	if (isset($result2)){ if (is_resource($result2)) mysqli_free_result($result2); }
