<?php
$result1 = mysqli_query($coneccion, 'SELECT id FROM inscribite_inscripciones WHERE deevento="' . $_GET['evento'] . '" LIMIT 1');
//$result1 = mysqli_query($coneccion,'SELECT id FROM inscribite_inscripciones WHERE deevento="'.$_GET['evento'].'" AND iniciadoeldia != "0000-00-00" ');
...
<div>
    ...
    <div>
        ...
        <?php
		$query = "SELECT op.evento,op.id,op.nombre,op.cupo,aux.inscripciones FROM inscribite_opciones op
	left join (select deevento,opcion,sum(1) inscripciones from inscribite_inscripciones group by deevento,opcion) aux on aux.deevento=op.evento and aux.opcion=op.nombre
	WHERE op.evento = '{$_GET['evento']}'";
        ...
        if ($opciones) {
            foreach ($opciones as $opcion) {
                ...
                if ($opcion['cupo'] != 0) {
                    echo 'Cupo restante: <strong>' . ($opcion['cupo'] - $opcion['inscripciones']) . '</strong> de ' . $opcion['cupo'] . '';
                }
                echo '<br/>';
            }
        }
        ...
        <br/>
        <?php
        if ($_GET['ordenarpor'] == 'inscribite_inscripciones.fecha')
            $_GET['ordenarpor'] = 'inscribite_inscripciones.pagoeldia';
            ...
            if ($inscripciones) {
                ...
                foreach ($inscripciones as $row1) {
                    ...
                    <tr>
                        ...
                        <td style="color:#666;font-size:11px;"><a title="codigo: <?= $row1['codigo'] ?>" href="?sec=inscripciones.editar&amp;editando=<?= $row1['id'] ?>"><?php
                                $arrnombresdpila = explode(' ', $row1['nombre']);
                                echo $arrnombresdpila[0] . ' ' . substr($arrnombresdpila[1], 0, 1);
                                if ($arrnombresdpila[1] != '')
                                    echo '.'
                                    ?></a></td>
                        <td style="color:#666;font-size:11px;"><a title="codigo: <?= $row1['codigo'] ?>" href="?sec=inscripciones.editar&amp;editando=<?= $row1['id'] ?>"><?=
                                ucwords(str_replace('autonoma', '', str_replace('autónoma', '', strtolower($row1['localidad']))));
                                if ($row1['localidad'] != '') {
                                ...
                                ?></a></td>
                        <td><a title="codigo: <?= $row1['codigo'] ?>" href="?sec=inscripciones.editar&amp;editando=<?= $row1['id'] ?>"><?php /* $result3 = mysqli_query($coneccion,'SELECT * FROM inscribite_categorias WHERE ((deevento = "'.$row1['deevento'].'") && (codigo = "'.substr($row1['codigo'], 4, 2).'")) LIMIT 1');
                        ...
                        <td><a href="javascript:confirm_entry('inscripcion de usuario: dni <?= $row1['deusuario'] ?>', 'inscripciones.admin<?php if ($_GET['evento'] != '') echo 'amp;evento=' . $_GET['evento'];
            if ($_GET['busqueda'] != '') echo 'amp;busqueda=' . $_GET['busqueda'];
            if ($_GET['ordenarpor'] != "") echo 'amp;ordenarpor=' . $_GET['ordenarpor'] ?>', 'inscribite_inscripciones', <?= $row1['id'] ?>)"> <img src="images/deletex.gif" alt="Eliminar"/></a></td>
                    </tr>
        <?php
