...
$result2 = mysql_query('SELECT nombre FROM '.pftables.'eventos WHERE codigo="'.$_GET['evento'].'" LIMIT 1 ');
$row2 = mysql_fetch_array($result2)?>
    <div class="columnacentral" style="height:auto;font-size:14px; width:100%;">
      <form action="inscripcion?evento=<?=$_GET['evento']?>" method="post">
        ...
        <input type="hidden" name="dianacimiento" value="<?=$_POST['dianacimiento']?>"/>
        <input type="hidden" name="mesnacimiento" value="<?=$_POST['mesnacimiento']?>"/>
        <input type="hidden" name="agnonacimiento" value="<?=$_POST['agnonacimiento']?>"/>
...
<?php include_once 'includes/_solofooter.php'?>