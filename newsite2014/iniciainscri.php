...
function mostrarselectmeses() {
    ...
    if ($_GET['modinscr'] != "")
        echo "&amp;modinscr=" . $_GET['modinscr'];
    if ($_GET['codigodedescuento'] != '')
        echo '&codigodedescuento=' . $_GET['codigodedescuento'];
    echo ';">' . chr(13);
...
function versiesmasbarato() {
    ...
    if (isset($_GET['modinscr'])) {
        ...
        $row1 = mysql_fetch_array($result1);
        if (($row1['pagado'] == 1) && ($row1['precio'] < $preciodecat))
            return true;
        else
            return false;
        mysql_free_result($result1);
...
?>
<div class="left">
    <h1>Inscripción a: <strong><?= $row1['nombre'] ?></strong></h1>
<?php
...
if (!($usuario != "")) {
    ...
    ?>
        <br/>Usted está logueado como <strong><?php getdatosusuario(); ?></strong><br/><br/><a href="cerrarsesion">Cerrar Sesion para registrar a otro usuario</a><br/><a href="registrate?usuario=<?= $usuario ?>">Ver o cambiar mis datos</a><br/><br/>
    <?php
    ...
    if (mysql_num_rows($result2) > 0) {
        ...
        <?=
        $row3['nombre'];
        if ((($row3['cupo'] + $row3['cuporestante']) <= 0) && ($row1['tipo'] != 'Servicios'))
            echo ' <span style=""> - No quedan vacantes disponibles en este grupo</span>' . chr(13);
        if ($row3['cupo'] != 0) {
        ...
        ?>
                ...
                while ($row2 = mysql_fetch_array($result2)) {
                    ...
                    ?>
                        <a id="linkinscr<?= $row2['id'] ?>"<?php if (!((($row3['cupo'] + $row3['cuporestante']) <= 0) && ($row1['tipo'] != 'Servicios'))) echo ' href="confirmarinscripcion?evento=' . $_GET['evento'] . '&amp;opcion=' . $row3['nombre'] . '&amp;cat=' . $row2['nombre'] . '&amp;cod=' . $row2['codigo'];
                echo '&amp;mes=' . substr('0' . $mesactual, -2, 2) . $anioactual;
                if ($_GET['modinscr'] != "") echo"&amp;modinscr=" . $_GET['modinscr'];echo($_GET['codigodedescuento'] != '') ? '&codigodedescuento=' . $_GET['codigodedescuento'] : '';
                echo '"'; ?>>
