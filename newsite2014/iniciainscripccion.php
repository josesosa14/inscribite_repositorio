    ...
    ?></h1>
        ...
        while ($row2 = mysql_fetch_array($result2)) {
            ...
            if ((($row2['sexo'] == 'Ambos') || (strtolower($row2['sexo']) == strtolower($sexodelusuario))) && ((($edadcomputable >= $row2['edadminima']) && ($edadcomputable <= $row2['edadmaxima'])) || ($row2['limitedeedad'] == 0)) && ((versiesmasbarato()) || ($row1['tipo'] == 'Servicios'))) {
                ...
                ?>
                                <a id="linkinscr<?= $row2['id'] ?>"<?php
                if (!((($row3['cupo'] + $row3['cuporestante']) <= 0) && ($row1['tipo'] != 'Servicios')))
                    echo ' href="confirmarinscripcion?evento=' . $_GET['evento'] . '&amp;opcion=' . $row3['nombre'] . '&amp;cat=' . $row2['nombre'] . '&amp;cod=' . $row2['codigo'];
                echo '&amp;mes=' . substr('0' . $mesactual, -2, 2) . $anioactual;
                if ($_GET['modinscr'] != "")
                    echo"&amp;modinscr=" . $_GET['modinscr'];
                echo($_GET['codigodedescuento'] != '') ? '&codigodedescuento=' . $_GET['codigodedescuento'] : '';
                echo '"';
                ?>>
