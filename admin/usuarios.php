...
-->
  ...
  <body>
    ...
    <div id="entradas"></div>
    <div id="cargando"></div>
    Ordenar por: <a href="usuarios?ordpor=nombre">nombre</a>,
    <a href="usuarios?ordpor=apellido<? if ($_GET['busqueda']!= "")echo "&amp;busqueda=".$_GET['busqueda']?>">apellido</a>,
    <a href="usuarios?ordpor=dni<? if ($_GET['busqueda']!= "")echo "&amp;busqueda=".$_GET['busqueda']?>">dni</a>,
    <a href="usuarios?ordpor=edad<? if ($_GET['busqueda']!= "")echo "&amp;busqueda=".$_GET['busqueda']?>">edad</a>,
    <a href="usuarios?ordpor=evento<? if ($_GET['busqueda']!= "")echo "&amp;busqueda=".$_GET['busqueda']?>">evento</a>,
    <a href="usuarios?ordpor=puntos<? if ($_GET['busqueda']!= "")echo "&amp;busqueda=".$_GET['busqueda']?>">puntos</a>
...
//mysql_query("ALTER TABLE `inscribite_usuarios` DROP `id` ");
//mysql_query("ALTER TABLE `inscribite_usuarios` ADD `id` INT(7 ) NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST ");
$result1 = mysql_query('SELECT id FROM inscribite_usuarios ');
$totalusuarios = mysql_num_rows($result1);
$nrohasta = ($_GET[verdesde]+100);
$botsiguiente = '<a href="usuarios?ordpor='.$_GET['ordpor'].'&verdesde = '.($_GET['verdesde']+100).'">Siguientes</a>';
...
if ($_GET['busqueda']!= "")
	...
	$result1 = mysql_query('SELECT * FROM inscribite_usuarios ORDER BY '.$ordpor.' LIMIT '.$verdesde.', 100 ');
...
while ($row1 = mysql_fetch_array($result1)){ ?>
      <tr>
        ...
        <td><? if ($row1['dni'] == $antedni)echo'<span style="color:red;">Se Repite</span>'?></td>
<?
    $antedni = $row1['dni'];
}
?>
